...
from model.evalAUC import evalAUC, checkAUC, acc_cal, auc4BCE
import sys
import random
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for input, label in test_loader:
                input, label = input.to(device), label.to(device).float()
                # label_onehot = F.one_hot(label, 2).float() 
                outputs = model(input) 
                # print(label_onehot)
                # print(outputs)
                # print(label)
                loss = criterion(outputs.squeeze(dim=-1), label)
                test_loss += loss.item()
            print('##################### Test #######################')
            print(f'Epoch {epoch+1}')
            print(f'test loss: {test_loss/len(test_loader)}.')
            test_auc = auc4BCE(model,device,X_test,y_test)
            if test_auc > max_auc:
...
x = torch.tensor([1.2,2.2,3.3]).float().reshape(3,1)
print(x.shape)
y = torch.tensor([1,2,4]).float().reshape(3,1)
print(x)
print(criterion(x,y))
